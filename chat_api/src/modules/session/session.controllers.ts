...
import { sessionInput, sessionResponseSchema, standardInput, standardResponseSchema, chatHistoryResponseSchema,  } from "./session.zodSchemas";
import { errorHandler } from "../../utils/errors";
import { Client } from "../../Client";
import { liveClients } from "../../index";
import { Message } from "../../Message";
import { Notification } from "../../Notification";
import { checkBlockStatusInDB, createNotificationInDB, createUserInDB, findChatHistoryInDB, findUserInDB, updateNotificationCreateFriendAndNotification, createFriendAndNotification, updateBlockStatusInDB } from "../../utils/dbUtils";

export async function initializeUserSession(request: FastifyRequest<{ Body: sessionInput }>, reply: FastifyReply) {
	try {
		const user = request.body as sessionInput;
		const userName = user.userName;
		const parsedUser = sessionResponseSchema.parse(user);
		if (liveClients.has(userName)) {
			console.log("Warning: Client already exists in memory");
			parsedUser.message = "Warning: Client already exists in memory to initialize";
			...
			if (existingClient) {
				...
				const friendList = new Set(existingClient.friendList.map((friend) => friend.friendName));
				const blockedList = new Set(existingClient.friendList
					  .filter(friend => friend.block === true)  // Filter out only blocked friends
					  .map(friend => friend.friendName));        // Extract the friend names
				const notifications = new Set(existingClient.notifications.map((notification) => new Notification(notification.notification, notification.pending)));
				const client = new Client(userName, friendList, blockedList, notifications);
				liveClients.set(userName, client);
				console.log("Client added to memory:", existingClient);
				parsedUser.message = "Client has been initialized from database";
...
export async function terminateUserSession(request: FastifyRequest<{ Body: sessionInput }>, reply: FastifyReply) {
	try {
		...
		if (!liveClients.has(userName)) {
			...
			parsedUser.message = "Warning: Client does not exist in memory to terminate";
...
export async function provideChatHistory(request: FastifyRequest<{ Body: standardInput }>, reply: FastifyReply) {
	try {
		...
		if (!client) {
			...
			parsedUser.message = "Warning: Client does not exist in memory to retrieve chat history";
			reply.code(409).send(parsedUser);
		}
		else {
			if (!client.getChatHistory().has(friendName)) {
				console.log("Chat history does not exist in memory. Fetching from database...");
				const chatHistory = await findChatHistoryInDB(userName, friendName);
				const messages = chatHistory.map((message) => new Message(message.message, message.sender));
				...
				parsedUser.message = "Chat history has been retrieved from database";
				reply.code(200).send(parsedUser);
			}
			else {
				console.log("Chat history exists in memory");
				parsedUser.chatHistory = client.getChatHistory().get(friendName)?.map((message) => ({
					message: message.getMessage(),
					sender: message.getSender(),
				})) || [];
				parsedUser.blockButton = client.isUserBlocked(friendName);
				parsedUser.message = "Chat history has been retrieved from memory";
				reply.code(200).send(parsedUser);
			}
		}
	} catch (error) {
...
export async function friendRequest(request: FastifyRequest<{ Body: standardInput }>, reply: FastifyReply) {
	try {
		const data = request.body as standardInput;
		const userName = data.userName;
		const friendName = data.friendName;
		const parsedUser = standardResponseSchema.parse(data);
		...
		if (!client) {
			console.log("Warning: Client does not exist in memory (not initialized)");
			parsedUser.message = "Warning: Client does not exist in memory to send friend request (not initialized)";
			reply.code(409).send(parsedUser);
			return;
		}
		...
		if(!blocked) {
			await createNotificationInDB(friendName, friendNameNotification, true);
		}
		
		client.getSocket()?.send(JSON.stringify({ notification: userNameNotification, pending: false, friendRequest: friendName }));
		if (friendClient) {
			if(!blocked) {
				friendClient.getSocket()?.send(JSON.stringify({ notification: friendNameNotification, pending: true, friendRequest: userName }));
			}
		}
...
export async function friendRequestAccepted(request: FastifyRequest<{ Body: standardInput }>, reply: FastifyReply) {
	try {
		const user = request.body as standardInput;
		const userName = user.userName;
		const friendName = user.friendName;
		const client = liveClients.get(userName);
		const parsedUser = standardResponseSchema.parse(user);
		if (!client) {
			console.log("Warning: Client does not exist in memory");
			parsedUser.message = "Warning: Client does not exist in memory to accept friend request";
			reply.code(409).send(parsedUser);
		}
		else {
			if (!client.isUserFriend(friendName)) {
				...
				await updateNotificationCreateFriendAndNotification(userName, friendName, userNameNotification, updateNotification, false);
				await createFriendAndNotification(friendName, userName, friendNameNotification, false);
				client.addToFriendList(friendName);
				client.getChatHistory().set(friendName, []);
				client.getSocket()?.send(JSON.stringify({ notification: userNameNotification , pending: false, friendRequestAccepted: friendName }));
				const friendClient = liveClients.get(friendName);
				if (friendClient) {
					friendClient.addToFriendList(userName);
					friendClient.getChatHistory().set(userName, []);
					friendClient.getSocket()?.send(JSON.stringify({ notification: friendNameNotification , pending: false, friendRequestAccepted: userName }));
				}
				console.log("Friend request accepted and saved to database");
				parsedUser.message = "Friend request accepted and saved to database";
				reply.code(201).send(parsedUser);
			}
			else {
...
export async function updateBlockStatus(request: FastifyRequest<{ Body: standardInput }>, reply: FastifyReply)
{
	try
	{
		const user = request.body as standardInput;
		...
		if (!client) {
			console.log("Warning: Client does not exist in memory");
			parsedUser.message = "Warning: Client does not exist in memory to update block status";
			reply.code(409).send(parsedUser);
		}
		else {
...
export async function inviteToPlay(request: FastifyRequest<{ Body: standardInput }>, reply: FastifyReply)
{
	try
	{
		const user = request.body as standardInput;
		...
		if (!client) {
			console.log("Warning: Client does not exist in memory");
			parsedUser.message = "Warning: Client does not exist in memory to invite to play";
			reply.code(409).send(parsedUser);
		}
		else {
			...
			if (client.isUserFriend(friendName) && !client.isUserBlocked(friendName)) {
				await createNotificationInDB(userName, userNameNotification, false);
				...
				client.getSocket()?.send(JSON.stringify({ notification: userNameNotification, pending: false, inviteToPlay: friendName }));
				if (friendClient) {
					if(!blocked) {
						friendClient.getSocket()?.send(JSON.stringify({ notification: friendNameNotification, pending: true, inviteToPlay: userName }));
					}
				}
...
export async function acceptPlayInvite(request: FastifyRequest<{ Body: standardInput }>, reply: FastifyReply)	
{
	try
	{
		const user = request.body as standardInput;
		...
		if (!client) {
			console.log("Warning: Client does not exist in memory");
			parsedUser.message = "Warning: Client does not exist in memory to accept invite";
			reply.code(409).send(parsedUser);
		}
		else {
			if (client.isUserFriend(friendName)) {
				...
				await updateNotificationCreateFriendAndNotification(userName, friendName, userNameNotification, updateNotification, false);
				await createFriendAndNotification(friendName, userName, friendNameNotification, false);
				client.getSocket()?.send(JSON.stringify({ notification: userNameNotification , pending: false, inviteAccepted: friendName }));
				const friendClient = liveClients.get(friendName);
				if (friendClient) {
					friendClient.getSocket()?.send(JSON.stringify({ notification: friendNameNotification , pending: false, inviteAccepted: userName }));
				}
