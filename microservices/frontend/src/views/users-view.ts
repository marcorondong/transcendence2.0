...
export class UsersView extends HTMLElement {
  ...
  userIsOnline(user: UserAggregated) {
    const onlineUser =
      this.chat.onlineUsers.find((onlineUser) => onlineUser.id === user.id) ||
  ...
  applyOnlineStatus() {
    ...
    icons.map((icon) => {
      ...
      if (status) {
        ...
        icon.classList.add("text-cyan-500", "glow-small", "bg-cyan-500/30");
        onlineStatusText.innerText = "online";
      } else {
        icon.classList.remove("text-cyan-500", "glow-small", "bg-cyan-500/30");
