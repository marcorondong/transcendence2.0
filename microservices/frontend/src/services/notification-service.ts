  ...
  listen() {
    ...
    document.addEventListener("notification", (e) => {
      if (e instanceof CustomEvent) {
        ...
        notificationType.classList.add("text-xs", "opacity-50", "uppercase");
        const notificationDate = document.createElement("div");
        notificationDate.classList.add("text-xs", "opacity-50");
        notificationDate.innerText = "Time: " + new Date().toLocaleTimeString();
        ...
        const messages = document.querySelectorAll(".pong-notification");
        messages.forEach((m) =>
