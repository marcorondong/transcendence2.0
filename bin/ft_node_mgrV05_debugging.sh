...
# ========================
prompt_for() {
	# printf "%s\n" "### INSIDE prompt_for()" >> "/tmp/marondon/transcendence/node_package_mgr.log" # TODO: Debugging
	printf "%s\n" "### INSIDE prompt_for() using variable LOG_FILE" >> "$LOG_FILE" # TODO: Debugging
	...
	printf "%s\n" "### INSIDE prompt_for() - Args parsed" >> "$LOG_FILE" # TODO: Debugging
...
' set -- $local_options
	...
	local_trimmed=""
	local_selected=""
	printf "%s\n" "### INSIDE prompt_for() - Split options" >> "$LOG_FILE" # TODO: Debugging
	...
	printf "%s\n" "### INSIDE prompt_for() - Validate default index: local_default: ${local_default}, local_total_opts: ${local_total_opts}" >> "$LOG_FILE" # TODO: Debugging
	...
	printf "%s\n" "### INSIDE prompt_for() Display question and options" >> "$LOG_FILE" # TODO: Debugging
	# Save original IFS
# 	OLDIFS=$IFS
# 	IFS='
# '
...
# 	done
	...
	while :; do
		printf "Your selection: "
		if ! IFS= read -r local_input < /dev/tty; then
			echo "❌ Cannot read user input. Aborting."
			exit 1
		fi
			...
			*)
				if [ "$local_trimmed" -ge 1 ] && [ "$local_trimmed" -le "$local_total_opts" ]; then
					...
					if [ "$local_return_type" = "value" ]; then
						...
						local_i=1
						for item in "$@"; do
							[ "$local_i" = "$local_selected" ] && echo "$item" && return 0
							local_i=$((local_i + 1))
						done
					else
						echo "$local_selected"
		...
		echo "❌ Invalid selection: \"$local_trimmed\". Please choose 1–$local_total_opts."
	done
}
...
choose_action() {
	printf "%s\n" "### INSIDE choose_action" >> "/tmp/marondon/transcendence/node_package_mgr.log" # TODO: Debugging
	action=$(prompt_for \
...
choose_target() {
	printf "%s\n" "### INSIDE choose_target" >> "/tmp/marondon/transcendence/node_package_mgr.log" # TODO: Debugging
...
##################################### MAIN #####################################
# printf "%s\n" "### Not started the Launch if no flags" >> "/tmp/marondon/transcendence/node_package_mgr.log" # TODO: Debugging
printf "%s\n" "### TESTING ###" >> "$LOG_FILE"
...
case "$ACTION" in
	...
	install|uninstall)
		printf "%s\n" "### INSIDE case ACTION in" >> "/tmp/marondon/transcendence/node_package_mgr.log" # TODO: Debugging
		handle_root
		find_services
